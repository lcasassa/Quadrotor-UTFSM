...
//	i2c_set_fast_mode(I2C2);
	 ...
	 * other masters.
	 */
	i2c_set_own_7bit_slave_address(I2C2, 0x02);
...
//printf("I2C_CR1(I2C1): 0x%x I2C_SR1(I2C1): 0x%x I2C_SR2(I2C1): 0x%x\r\n", I2C_CR1(I2C1), I2C_SR1(I2C1), I2C_SR2(I2C1));
	...
	timeout=TIMEOUT_VALUE;
        while (!((I2C_SR1(I2C2) & I2C_SR1_SB)
                & (I2C_SR2(I2C2) & (I2C_SR2_MSL | I2C_SR2_BUSY))) && timeout!=0) timeout--;

//	if(timeout==0) printf("I2C_CR1(I2C1): 0x%x I2C_SR1(I2C1): 0x%x I2C_SR2(I2C1): 0x%x\r\n", I2C_CR1(I2C1), I2C_SR1(I2C1), I2C_SR2(I2C1));
	if(timeout==0) { return -1; }

        /* Send destination address. */
        i2c_send_7bit_address(I2C2, sla, I2C_WRITE);

        /* Waiting for address is transferred. */
	timeout=TIMEOUT_VALUE;
	while (!(I2C_SR1(I2C2) & I2C_SR1_ADDR) && timeout!=0) {
		timeout--;
...
*/
	...
	if(timeout==0) { return -3; }
        ...
        /* Sending the data. */
        i2c_send_data(I2C2, reg_addr); /* stts75 config register */

	timeout=TIMEOUT_VALUE;
        while (!(I2C_SR1(I2C2) & I2C_SR1_BTF) && timeout!=0) timeout--; /* Await ByteTransferedFlag. */
	if(timeout==0) { return -4; }

        /* Polarity reverse - LED glows if temp is below Tos/Thyst. */
        i2c_send_data(I2C2, value);

	timeout=TIMEOUT_VALUE;
        while (!(I2C_SR1(I2C2) & (I2C_SR1_BTF | I2C_SR1_TxE)) && timeout!=0) timeout--;
...
int i2c2_read(char sla, char reg_addr, char * value, char len) {
	...
	u32 timeout;
        ...
        /* Waiting for START is send and switched to master mode. */
	timeout=TIMEOUT_VALUE;
        while (!((I2C_SR1(I2C2) & I2C_SR1_SB) & (I2C_SR2(I2C2) & (I2C_SR2_MSL | I2C_SR2_BUSY))) && timeout!=0) timeout--;
	if(timeout==0) { return -1; }
...
*/
	...
	if(timeout==0) { return -3; }
        ...
        i2c_send_data(I2C2, reg_addr); /* temperature register */
	timeout=TIMEOUT_VALUE;
        while (!(I2C_SR1(I2C2) & (I2C_SR1_BTF | I2C_SR1_TxE)) && timeout!=0) timeout--;
	if(timeout==0) { return -4; }
	...
	if(timeout==0) { return -5; }
        ...
        /* Say to what address we want to talk to. */
        i2c_send_7bit_address(I2C2, sla, I2C_READ);
...
*/
	...
	timeout=TIMEOUT_VALUE;
        while (!(I2C_SR1(I2C2) & I2C_SR1_BTF) && timeout!=0) timeout--;
...
void i2c2_recover() {
	...
	// SCL HIGH
	gpio_set_mode(GPIOB, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT, GPIO_I2C2_SCL);

	timeout=0;
	while (!gpio_get(GPIOB, GPIO_I2C2_SDA)) { // if SDA == LOW continue with clocks

		if(timeout++ >= 1000) {
			printf("i2c2_recover() timeout..\r\n");
			break;
		}
		// SCL LOW
